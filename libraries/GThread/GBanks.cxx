...
std::ostream& operator<<(std::ostream& os, const G4S800 &s800pack){
...
std::ostream& operator<<(std::ostream& os, const G4SimPacket &packet){
...
Int_t GEBMode3Head::GetLength()     const { return  lengthGA&0x07ff; }
Int_t GEBMode3Head::GetChannel()    const { return  board_id&0x000f; }
Int_t GEBMode3Head::GetVME()        const { return (board_id&0x0030)>>4; }
Int_t GEBMode3Head::GetCrystal()    const { return (board_id&0x00c0)>>6; }
Int_t GEBMode3Head::GetHole()       const { return (board_id&0x1f00)>>8 ; }
Int_t GEBMode3Head::GetSegmentId()  const { return GetVME()*10+GetChannel(); }
Int_t GEBMode3Head::GetCrystalId()  const { return GetHole()*4+GetCrystal(); }
...
std::ostream& operator<<(std::ostream& os,const S800TriggerPacket &pack) {
  ...
  for(int x=0;x<pack.channel_time_number;x++) {
    os << "\t" << x
       << "Tigger[" << GetS800Channel(pack.channel_time[x]) << "]   "
       << GetS800Value(pack.channel_time[x]) << "\n";
  }
  return os;
}
...
std::ostream& operator<<(std::ostream& os,const S800TOFPacket &tof) {
  ...
  for(int x=0;x<tof.number;x++) {
    switch(GetS800Channel(tof.value[x])) {
      case 12:
        os << "\t RF:  0x" << std::hex << GetS800Value(tof.value[x]) << std::dec << "\n";
        break;
      case 13:
        os << "\t Object Scintillator:  0x" << std::hex << GetS800Value(tof.value[x]) << std::dec << "\n";
        break;
      case 14:
        os << "\t XFP Scintillator:  0x" << std::hex << GetS800Value(tof.value[x]) << std::dec << "\n";
        break;
      case 15:
        os << "\t Si:  0x" << std::hex << GetS800Value(tof.value[x]) << std::dec << "\n";
        break;
      default:
        os << "\t " << GetS800Channel(tof.value[x]) << ":  0x" << std::hex << GetS800Value(tof.value[x]) << std::dec << "\n";
        break;
...
Long_t GEBMode3Data::GetLed() const { return (((long)led_high)<<32) + (((long)led_middle)<<16) + (((long)led_low)<<0); }
Long_t GEBMode3Data::GetCfd() const { return (((long)cfd_high)<<32) + (((long)cfd_middle)<<16) + (((long)cfd_low)<<0); }
UShort_t GEBMode3Data::GetDeltaT1() const { return cfd_low; }
UShort_t GEBMode3Data::GetDeltaT2() const { return cfd_middle; }


Int_t  GEBMode3Data::GetEnergy(GEBMode3Head &head) const  {
...
Int_t  GEBMode3Data::GetEnergy0(GEBMode3Head &head) const  {
  int channel = head.GetChannel();
  //if ((channel==1) &&( head.GetHole()==9) && (head.GetCrystal()==3) && (head.GetVME()==3))
    //channel = 9; //  Q5 e5 has an inverted radial box, treat it as a core.  pcb.
...
Int_t  GEBMode3Data::GetEnergy1(GEBMode3Head &head) const  {
  int channel = head.GetChannel();
  //if ((channel==1) &&( head.GetHole()==9) && (head.GetCrystal()==3) && (head.GetVME()==3))
    //channel = 9; //  Q5 e5 has an inverted radial box, treat it as a core.  pcb.
...
Int_t  GEBMode3Data::GetEnergy2(GEBMode3Head &head) const  {
  int channel = head.GetChannel();
  //if ((channel==1) &&( head.GetHole()==9) && (head.GetCrystal()==3) && (head.GetVME()==3))
    //channel = 9; //  Q5 e5 has an inverted radial box, treat it as a core.  pcb.
...
std::ostream& operator<<(std::ostream& os, const TNSCLBeginRun& begin) {
